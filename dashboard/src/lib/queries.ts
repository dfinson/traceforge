...
// `GET /api/runs` returns every run fully assembled, and every view aggregates
// client-side over that shared array — exactly as the approved mock did against
// its synthetic `RUNS` const. React Query dedupes by query key, so the many
// components that call `useRuns()` (Fleet, its charts, RunView, Triage, Cost,
// Coverage) share a single fetch + cache entry.
