...
This is the one that matters most. **Correction (verified on disk):** VS Code Copilot
...
**Where it lands (verified — this is the rich, complete record):**
- `%APPDATA%\Code\User\workspaceStorage\<workspace-hash>\chatSessions\<sessionId>.jsonl`
  — VS Code's **ChatModel journal** (`version: 3`). It is line-delimited JSONL but
  **not** the CLI event format: line 0 is a full snapshot `{kind:0, v:{requests:[],…}}`
  and each later line is a JSON-patch record `{kind:1, k:[path], v}` (set) or
  `{kind:2, …}` (append). Replaying the journal reconstructs `requests[]`, where each
  request carries the user `message`, the `response` parts, and tool invocations.
- Index/metadata (titles, timing, session ids) live in the workspace
  `state.vscdb` under keys `chat.ChatSessionStore.index` and `agentSessions.state.cache`
  (`vscode-chat-session://local/<base64-id>` resources).

**About markdown:** VS Code does not *persist* the log as markdown. Markdown appears only
(a) as the body text *inside* the assistant `response` parts, and (b) if you run the
**`Chat: Export Session`** command, which writes a `.md` you choose the path for. The
exported `.md` is lossy (no structured tool args/results), so we ingest the `.jsonl`
journal instead.

> The thin `github.copilot-chat\session-store.db` `turns` table (rows with
> `host_type='vscode'`) only captures a final user/assistant text pair and only when
> chat delegates to the bundled CLI — it is a last-resort fallback, mapped via
> `copilot_markdown.yaml`, not the primary source.

**Hand-off:** the freshest journal in the scratch workspace is the session you just ran.

```powershell
Get-ChildItem "$env:APPDATA\Code\User\workspaceStorage\*\chatSessions\*.jsonl" |
