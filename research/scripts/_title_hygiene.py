"""Decode/render hygiene for generated titles (shared by judge + e2e renderers).

The model is clean in-distribution but degenerates in the zero-shot regime
(unseen trace families, intent-less context): verbatim repetition, beams that
end mid-subword ("...restarte"), and child titles that duplicate their parent.
These are decode/render defects, not data defects, so they are fixed here
post-generation rather than by retraining.

Functions:
  clean_title(s)        -> normalise whitespace/punctuation, collapse repeats.
  best_of(cands)        -> pick the first non-degenerate beam alternate, cleaned.
  norm_key(s)           -> plural/stopword-insensitive token-set key so a child
                           title that merely restates its parent (or a sibling)
                           is detected and the next alternate is used.
  pick_distinct(used, cands) -> best_of, but skipping alternates whose norm_key
                           was already used in this session (parent/child/sibling
                           de-dup); falls back to best_of if all collide.
"""
