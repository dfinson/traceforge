...
class OrtTitler:
    def __init__(self, model_dir, tok_dir=None, q=True, threads=2,
    ...
    def generate(self, text, num_beams=5, num_return=1, max_new=32,
                 no_repeat=2, rep_pen=1.3, len_pen=0.8):
        ...
        if len(done) < num_return:
            tail = sorted(((s / (len(t) ** len_pen), t) for s, t in zip(scores, beams)),
