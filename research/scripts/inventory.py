...
print(
    f"  openhands:  {len(oh_ext)} sessions, {sum(len(e.get('events', [])) for e in oh_ext)} events"
)
print(
    f"  swesmith:   {len(ss_ext)} sessions, {sum(len(e.get('events', [])) for e in ss_ext)} events"
)
print()
...
print(
    f"  no-run labels matching swe base_instance_id:       {sum(1 for k in labeled_no_run if k in swe_base_ids)}"
)
print(
    f"  no-run labels matching openhands base_instance_id: {sum(1 for k in labeled_no_run if k in oh_base_ids)}"
)
print(
    f"  no-run labels matching swesmith base_instance_id:  {sum(1 for k in labeled_no_run if k in ss_base_ids)}"
)
print()
