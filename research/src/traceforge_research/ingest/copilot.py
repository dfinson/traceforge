...
async def _ingest_sessions(
    ...
    # traceforge dep graph for callers that just want config.
...
def _resolve_mapping_path() -> Path:
    """Locate the bundled copilot_markdown.yaml inside the installed traceforge."""

    import traceforge

    pkg_dir = Path(traceforge.__file__).resolve().parent
