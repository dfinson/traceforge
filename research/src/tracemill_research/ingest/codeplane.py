"""Harvest CodePlane trail-node gold into the tracemill distillation corpus.
...
title gold the tracemill titler is missing — see the request-titler leak audit
...
:class:`~tracemill.types.SessionEvent` objects (an ``agent_message`` narration
...
:func:`~tracemill.title.context.distilled_context` projection. The gold
...
Never re-ingest labelled tracemill sessions: CodePlane is a **new** source with
