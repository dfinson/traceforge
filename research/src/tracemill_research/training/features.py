...
:mod:`tracemill.phase.features` so a single implementation serves both training
...
from tracemill.phase.features import (
    MAX_TEXT_CHARS,
    MODEL2VEC_DIM,
    MODEL2VEC_NAME,
    PHASES,
...
from tracemill.boundary.features import GapExample, featurize_session_gaps
...
def load_phase_examples(
    ...
    :func:`tracemill.phase.features.featurize_session_events` (shared with
    ...
    instead (see ``tracemill.phase.inference.predict_session``). Featurisation
...
def load_boundary_examples(seg_params=None) -> list[GapExample]:
    ...
    (:func:`tracemill.boundary.features.featurize_session_gaps`) so training and
