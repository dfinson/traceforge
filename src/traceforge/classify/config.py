...
2. ``TRACEFORGE_CONFIG`` environment variable
3. ``.traceforge/config.yaml`` in project directory (cwd upward)
4. ``~/.config/traceforge/config.yaml`` (user-global)
5. Entry points: ``traceforge.profiles`` group
...
from traceforge.classify.phases import with_phase_map
...
def _find_project_config() -> Path | None:
    """Walk cwd upward looking for .traceforge/config.yaml."""
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / ".traceforge" / "config.yaml"
        if candidate.is_file():
            return candidate
        # Also check config.yml
        candidate_yml = parent / ".traceforge" / "config.yml"
...
def _find_user_config() -> Path | None:
    """Find user-global config at ~/.config/traceforge/config.yaml."""
    config_home = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config"))
    for ext in ("yaml", "yml"):
        candidate = config_home / "traceforge" / f"config.{ext}"
        if candidate.is_file():
            return candidate
    return None
...
def _load_entry_point_configs() -> list[dict[str, Any]]:
    ...
    try:
        eps = importlib.metadata.entry_points(group="traceforge.profiles")
    except TypeError:
        # Python 3.11 compatibility
        eps = importlib.metadata.entry_points().get("traceforge.profiles", [])
...
def _load_builtin_defaults() -> dict[str, Any]:
    ...
    merged: dict[str, Any] = {}
    data_dir = importlib.resources.files("traceforge.classify") / "data"
    for filename in (
...
def load_config(
    ...
    *,
    ...
    if env_path and Path(env_path).is_file():
        logger.debug("Loading config from TRACEFORGE_CONFIG=%s", env_path)
    ...
    # top-level keys for other traceforge subsystems like log_level, sdk, etc.)
...
class ClassificationEngine:
    ...
    def __init__(self, config: ClassifyConfig) -> None:
        from traceforge.classify.mcp import McpServerProfile, McpToolOverride
        from traceforge.classify.rules import BinaryInfo, Rule, ShellActivity
        ...
        # Binary info
        from traceforge.classify.core import Effect as EffectEnum
