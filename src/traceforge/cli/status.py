"""Status command — show traceforge system state."""
...
_DB_PATH = Path.home() / ".traceforge" / "system.db"
...
def status(as_json: bool, db_path: str | None) -> None:
    """Show traceforge system state from the governance database."""
    path = Path(db_path) if db_path else _DB_PATH
    if not path.exists():
        click.echo("No system database found. Has traceforge run yet?")
        sys.exit(1)
    ...
    if as_json:
        ...
        click.echo("Traceforge System Status")
        click.echo("─" * 40)
