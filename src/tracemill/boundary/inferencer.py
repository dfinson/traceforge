...
through :class:`tracemill.boundary.decode.StreamingBoundaryDecoder`, and the
...
Mirrors :mod:`tracemill.phase.inferencer`. The model is the only boundary
...
class BoundaryInferencer:
    ...
    :func:`tracemill.boundary.inference.resolve_model_path` (env var / packaged
...
class SessionBoundaryStream:
    ...
    Drives :class:`~tracemill.phase.segmentation.IncrementalSegmentation` so each
    ...
    refractory :class:`~tracemill.boundary.decode.StreamingBoundaryDecoder`, and
    ...
    def __init__(self, inferencer: "BoundaryInferencer", session_id: str, source: str) -> None:
        from tracemill.phase.segmentation import IncrementalSegmentation
