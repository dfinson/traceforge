...
_DEFAULT_CONFIG_PATH = Path.home() / ".tracemill" / "config.yaml"


@click.group()
def config() -> None:
    """Manage tracemill configuration."""


@config.command()
@click.option("--force", is_flag=True, help="Overwrite existing config file.")
def init(force: bool) -> None:
    """Write default config to ~/.tracemill/config.yaml."""
...
def show(config_path: str | None) -> None:
    ...
    if path is None or not path.exists():
        click.echo("No config file found. Run `tracemill config init` to create one.")
        sys.exit(1)
