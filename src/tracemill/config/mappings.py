...
Search order:
  ...
  3. Bundled mappings (src/tracemill/mappings/) — shipped with package
...
def _get_bundled_dir() -> Path:
    ...
    if _BUNDLED_MAPPINGS_DIR is None:
        ref = importlib.resources.files("tracemill") / "mappings"
...
def resolve_mapping_path(name: str, extra_dirs: list[Path] | None = None) -> Path | None:
    ...
    user_mappings = Path.home() / ".tracemill" / "mappings"
