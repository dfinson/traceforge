...
class MCPIntegrityAlert:
    ...
    server: str
    alert_type: Literal[
        "effect_escalation", "capability_gain", "scope_expansion",
        "description_change", "schema_change", "adversarial_pattern"
    ]
    previous: str
    current: str
...
_DANGEROUS_CAPS = frozenset({"network_outbound", "elevated_privilege", "arbitrary_execution", "credential_exposure"})
...
class MCPIntegrityScanner:
    ...
    def scan(self, ctx: "EnrichmentContext", cap: set[str]) -> tuple[list[MCPIntegrityAlert], bool]:
        ...
        if stored["description_hash"] != desc_hash:
            alerts.append(MCPIntegrityAlert(
    ...
    def check_semantic_drift(
        ...
        if reg_effect and current.effect:
            ...
            if curr_order > prev_order:
                ...
                alerts.append(MCPIntegrityAlert(
                    tool_name=tool, server=server,
                    alert_type="effect_escalation",
                    previous=reg_effect, current=current.effect,
    ...
    def scan_description(
        self, description: str, tool_name: str, server: str, now: datetime,
    ) -> list[MCPIntegrityAlert]:
