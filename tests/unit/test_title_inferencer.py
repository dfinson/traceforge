...
def _event(i, tool="edit", boundary=None, payload=None, kind="tool.call", metadata=True):
    ...
    return SessionEvent(
        id=f"e{i}", kind=kind, session_id="S",
...
_HAS_DEPS = (importlib.util.find_spec("onnxruntime") is not None
             and importlib.util.find_spec("tokenizers") is not None)
_HAS_MODEL = (Path(__file__).resolve().parents[2]
              / "src/tracemill/title/data/encoder.onnx").exists()


@pytest.mark.skipif(not (_HAS_DEPS and _HAS_MODEL),
                    reason="title extra / model artifacts absent")
def test_real_model_produces_distinct_nonempty_titles():
    ...
    stream.push(_event(2, tool="shell", boundary="step-boundary",
                       payload={"arguments": {"command": "pytest"}}))
    _e3, updates = stream.push(_event(3, tool="report_intent", boundary="activity-boundary",
