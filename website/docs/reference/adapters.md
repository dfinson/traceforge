...
| `gate_langchain(tool)` | the same `tool` (its `_run` is wrapped; `_arun` too for async-native tools) | Wraps the tool's `_run`. Native-async tools (a non-`None` `coroutine` slot, or an overridden `_arun`) are also gated on `_arun`, so async calls (`_arun` / `ainvoke`) are covered; sync-only `StructuredTool`s whose async path routes back through `_run` are detected and skipped to avoid double-gating. DENY → raises `ToolException`. Idempotent. | invocation `config["configurable"]["thread_id"]`, else `"langchain"` |
...
| `gate_openai_agents(agent)` | the `agent` (each `FunctionTool`'s `on_invoke_tool` is wrapped) | Wraps each `FunctionTool`'s `on_invoke_tool` for **real per-tool gating**: the real `tool_name` reaches the gate. DENY → raises `RuntimeError("Denied: …")` before the tool body runs (fails closed; a scorer/gate error also denies). Postflight runs on the tool result — SUPPRESS/REDACT rewrite the output, TERMINATE raises. Only `FunctionTool`s are wrapped (hosted/handoff tools skipped). Idempotent (per-agent and per-tool markers). | `agent.name`, else `"openai_agents"` |
